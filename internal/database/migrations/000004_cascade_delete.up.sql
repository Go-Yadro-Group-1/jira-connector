-- raw.status_changes → raw.issue (cascade: удаление issue → удаление status_changes)
ALTER TABLE raw.status_changes
    DROP CONSTRAINT status_changes_issue_id_fkey,
    ADD CONSTRAINT status_changes_issue_id_fkey
        FOREIGN KEY (issue_id) REFERENCES raw.issue(id) ON DELETE CASCADE;

-- raw.issue → raw.project (cascade: удаление project → удаление issue)
ALTER TABLE raw.issue
    DROP CONSTRAINT issue_project_id_fkey,
    ADD CONSTRAINT issue_project_id_fkey
        FOREIGN KEY (project_id) REFERENCES raw.project(id) ON DELETE CASCADE;

-- analytics tables → raw.project (cascade)
ALTER TABLE analytics.open_task_time
    ...
    ADD CONSTRAINT open_task_time_id_project_fkey
        FOREIGN KEY (id_project) REFERENCES raw.project(id) ON DELETE CASCADE;
